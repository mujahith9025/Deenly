    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="text-xs px-3.5 py-1.5 rounded-full bg-surface-container border border-outline-variant/40 text-secondary hover:border-primary transition self-start sm:self-auto shrink-0 cursor-pointer"
        >
      ...
      {showFullView ? (
        <div className="space-y-6">
          {/* Daily Goal & Continue Reading Dual Hero */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {/* Daily Goal Progress Card */}
            <div className="p-6 rounded-3xl glass-card border border-outline-variant/30 space-y-4 relative overflow-hidden flex flex-col justify-between">
            ...
            <div className="md:col-span-2 p-6 rounded-3xl bg-surface-container-low border border-outline-variant/40 relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          ...
          {/* Timeframe Filter Tabs & 4 Stat Cards */}
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {/* Streak Card */}
              <div className="p-4 md:p-5 rounded-2xl glass-card border border-outline-variant/30 relative overflow-hidden">
              ...
              {/* Verses Recited */}
              <div className="p-4 md:p-5 rounded-2xl glass-card border border-outline-variant/30">
                <div className="flex items-center justify-between mb-2">
          ...
          {/* Desktop Multi-Column: Main Habits/Streak Column (2/3) + Right Rail (1/3) */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            {/* Left/Main Column: Habits Checklist & Weekly Consistency */}
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="p-6 rounded-3xl glass-card border border-outline-variant/30 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold font-h2 text-on-surface">Weekly Consistency</h3>
                  <span className="text-xs text-secondary font-medium">{dailyGoalVerses} verses/day goal</span>
                </div>
                ...
                <div className="grid grid-cols-7 gap-2 py-2">
                  {weekDays.map((wd, i) => (
                    <div key={i} className="flex flex-col items-center gap-1.5">
                      <span className="text-[10px] text-outline">{wd.day}</span>
                      <div
            ...
            {/* Right Rail (Desktop Dedicated Column): Suggested Surahs & Khatm Progress */}
            <div className="space-y-6">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-on-surface-variant">{s.tag}</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-2">
                        <span className="font-noto-serif text-sm text-primary-fixed-dim">{s.arabicName}</span>
                        <ChevronRight className="w-3.5 h-3.5 text-outline group-hover:text-primary transition" />
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {/* Khatm Goal Progress Card */}
              <div className="p-6 rounded-3xl glass-card border border-outline-variant/30 space-y-4">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1.5 font-semibold text-on-surface">
                    <TrendingUp className="w-4 h-4 text-secondary" />
                    <span className="text-sm font-bold font-h2">Khatm Completion</span>
                  </div>
                  <span className="font-bold text-secondary text-sm">{khatmPercent}%</span>
                </div>
                ...
                <div className="w-full bg-surface-container-highest h-2.5 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${Math.max(khatmPercent, 4)}%` }}
                  />
                </div>

                <div className="p-3.5 rounded-2xl bg-surface-container/60 border border-outline-variant/20 text-xs flex items-center justify-between">
                  <div>
                    <p className="text-[11px] text-outline">Total Quran Pages</p>
                    <p className="font-bold text-on-surface">{user?.pages || 0} / 604 pgs</p>
                  </div>
        ...
        <ScreenPlaceholder
          ...
          description="Main user command center featuring daily prayer times, Islamic habit checklist, weekly consistency tracker, Hasanat points, and last-read Quran progress."
          stitchScreenName="Deenly Home Dashboard / Deenly Dashboard - Desktop / Deenly Dashboard - Tablet"
          stitchScreenId="1e094c856565484493b528b80ddfcae2 / a91bfa3067af4551bae33699954497df"
          stitchReady={true}
          currentRoute="/dashboard"
          featuresList={[
            'Daily Goal Card showing live X / 10 verses progress and completion badge',
            'Deep-linked Resume Reading shortcut jumping to exact saved position',
            'Dedicated desktop right rail for Suggested Surahs and Khatm completion progress',
            'Single 4-column metric row on tablet viewports and 2x2 grid on mobile',
            'Dynamic 7-day consistency calendar reflecting daily goal achievements',
          ]}
        />
      )}
    </div>
