...
- **Dashboard Screen (`/dashboard`)**: Dynamic greeting with real-time target status (`12/20 Ayahs • 60%`), Hero Recitation & Weekly Consistency cards, **Interactive Digital Tasbih dial positioned in the middle below Spiritual Metrics with prominent 1-Tap "Focus Mode" / "முழுத்திரை" buttons with text, Aggregate Daily Goal status (`todayTotal/800`), Auto-advance upon Dhikr completion, and Mid-recitation pause & count hold**, **PWA Install Banner & Quick Install Button** for devices running in web browser mode, and **Daily Inspirations (Verse of the Day & Hadith of the Day)** positioned below with prominent chapter/verse numbering and canonical Hadith badges.
- **Explore Hub (`/explore`)**: Sleek **compact rectangular category cards** (`Tasbih & Dhikr`, `Hisnul Muslim (132 Duas)`, `99 Names of Allah`), **Top-Positioned Dhikr Counter**, **Full-Screen Blind-Tap Focus Mode (Whole-Screen Touch Counter with Screen Wake-Lock, Live Bilingual Phonetics & Translation Switcher `English` ↔ `தமிழ்` ↔ `Hide`, Auto-Advance on Target Completion, and clean one-shot tap ripple indication only on active tap)**, **Custom Dhikr & Personal Dua Creator (Add, Edit, and Count personal Duas with custom targets, Arabic text, and translations)**, **Universal Goal Multiplier (`100/dhikr` $\rightarrow$ `800 Daily Goal` across 8 Dhikrs)**, **Mid-Recitation State Hold & Resume**, instant full-library view displaying presets + custom Dhikrs without category tabs, **single-tap Translation, Phonetics & Hadith Virtue toggle** (`English` ↔ `தமிழ்`), **Universal Target Setting** across all presets, real-time **Daily Progress** percentage calculation, and streamlined Dhikr analytics (7/14/30-day charts, daily targets, and Dhikr distribution breakdown with punchy headings: *"Your Most Recited Dhikrs"* and *"Daily Dhikr Volume"*).
- **Quran Catalog (`/quran`)**: High-contrast summary metrics (`114 Surahs • 6,236 Ayahs • 30 Juz`), modern Quran Surah cards with squircle numbers and calligraphy, streamlined action controls (`▶ Play Surah` and Jump to Verse), and contextual translation toggle available inside specific chapter views.
- **Quran Reading Screen (`/reading`)**: **Balanced fixed header & footer with prominent bookmark visibility**, monospace session timer, live letter rewards, dynamic font-size scrolling down to 54px, **Instant reflection of Hasanat, reading time, and verses across Dashboard & Profile upon clicking "I'm Done"**, **smooth stable non-oscillating reading position flow**, clean bilingual translation badges (`English • Sahih` / `தமிழ் • பாகவி`), **5 Mushaf Eye-Comfort Themes** with punchy single-line descriptions, and **Tactile Haptic Feedback** on Ayah navigation, bookmarking, and goal completions.
