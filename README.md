...
- **Dashboard Screen (`/dashboard`)**: Dynamic greeting with real-time target status (`12/20 Ayahs • 60%`), **1-Tap "Resume Recitation" Hero Card** displaying active Surah, Ayah, Juz, and reading streak, **Instant Quick Actions Dock** with 1-tap shortcuts (`Quran`, `Tasbih`, `Daily Duas`, `Saved Bookmarks`, `Hadith`), Hero Recitation & Weekly Consistency cards, **Interactive Digital Tasbih dial positioned in the middle below Spiritual Metrics**, **PWA Install Banner & Quick Install Button** for devices running in web browser mode, and **Daily Inspirations (Verse of the Day & Hadith of the Day)** positioned below with prominent chapter/verse numbering and canonical Hadith badges.
