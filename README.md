...
- **Explore Hub (`/explore`)**: Sleek **compact rectangular category cards** (`Tasbih & Dhikr`, `Hisnul Muslim (132 Duas)`, `99 Names of Allah`) matching the clean Hadith design with immediate direct access, **Universal Target Setting** across all presets, real-time **Daily Progress** percentage calculation, and **Bilingual Hadith Virtues** (`English` | `தமிழ்` | `Dual`).
...
- **Hadith Collections (`/hadith`)**: Instant search-first layout with clean, compact scholarly book cards (descriptions removed for rapid scanning), smart interval search, and concise Hadith reader stream with segmented translation switchers (`English`, `தமிழ்`, `Dual`).
